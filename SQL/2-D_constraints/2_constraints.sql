...
alter TABLE Employees add CONSTRAINT gt_z check(EmployeesId>0);
...
alter TABLE Departments add CONSTRAINT id_pk PRIMARY key(DepartmentID); --error
/*Msg 8111, Level 16, State 1, Line 1
Cannot define PRIMARY KEY constraint on nullable column in table 'Departments'.*/

--5
alter TABLE Departments add CONSTRAINT uq_name UNIQUE(DepartmentName);
...
--7
alter TABLE Projects add CONSTRAINT ch_len check (len(ProjectName)>0)
...
--9
alter table Salaries add CONSTRAINT am_ch check(Amount>0)


--10
alter TABLE Salaries add CONSTRAINT df_am   DEFAULT 50000 for Amount;
...
--2
ALTER TABLE Timesheets add CONSTRAINT hr_worked check(HoursWorked>=0 and HoursWorked<24);
...
-- 4
ALTER TABLE Departments add CONSTRAINT not_str_deptName check(DepartmentName not LIKE '%[0-9]%')
...
--6
ALTER TABLE Leaves add CONSTRAINT df_stDate DEFAULT GETDATE() for StartDate;
...
--2
ALTER TABLE Reviews add CONSTRAINT out_rng CHECK(Score>=1 and Score<=5);
